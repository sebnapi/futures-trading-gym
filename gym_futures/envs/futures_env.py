...
class FuturesEnv(gym.Env):
    ...
    def __init__(self, states: Sequence[TimeSeriesState], value_per_tick: float,
        ...
        self.minimum_price_movement = self.value_per_tick
    ...
    def get_episode_data(self, return_states:bool = False) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
    ...
    def generate_episode_metrics(self, as_file: bool=True) -> dict:
        ...
        profit_factor = abs(win_pl) / abs(loss_pl) if l_loss_pl != 0 else 1.
        ...
        vars_ = {
            ...
            'pnl_monotonicity': monotonicity(np.array(running_pnl)),
        ...
        if as_file:
            with open(f"{self.log_dir}/metrics/metrics_episode_{str(self.episode)}.json", "w+") as _file:
                _file.write(json.dumps(vars_, indent=4, cls=NumpyEncoder))
        return vars_
